...
export default class AttendeeSelector extends PureComponent {
  ...
  updateTiersAndGroups = props => {
    const getById = prop =>
      this.props[prop].reduce((byId, obj) => {
        byId[obj.id] = obj
        return byId
      }, {})
  ...
  render() {
    ...
    return (
      <div>
        ...
        {type === 'textCSV' || type === 'webCSV' || type === 'videoCSV' ? (
          <p>{t('unavailableCSV')}</p>
        ) : (
          <div className="attendee-selector">
            <div className="attendee-selector__menu">
              ...
              <div
                className={this.classNameForMenuItem('attendees')}
                onClick={this.viewAllAttendees}
              >
                {t('allAttendees')}
              </div>
              {/* <div className={this.classNameForMenuItem('tiers')} onClick={this.viewTiers}>
                {t('tiers')}
              </div> */}
              <div className={this.classNameForMenuItem('groups')} onClick={this.viewGroups}>
                {t('groups')}
              </div>
            </div>
              ...
              <thead>
                {view === 'attendees' ? (
                  <tr>
                    ...
                    <td className="attendee-selector__column__space">
                      <input
                        className="attendee-selector__search"
                        type="text"
                        placeholder="Search"
                        value={search}
                    ...
                    <td className="attendee-selector__column" />
                  </tr>
                ) : (
                  <tr>
                    <td>&nbsp;</td>
  ...
  renderTableRows = () => {
    ...
    switch (this.state.view) {
      case 'attendees':
        ...
        return this.state.attendees.map(a => {
          ...
          return (
            <tr
              key={a.id}
              className={`attendee-selector__attendee${inTierOrGroup ? '--disabled' : ''}`}
            >
              <td>
                {!inTierOrGroup && (
                  <input
                    ...
                    onChange={this.onAttendeeChange(a.id)}
                  />
                )}
              </td>
              <td className="attendee-selector__name">
                {a.firstName} {a.lastName}
              </td>
              <td>
                {/* {this.tiers[a.tierId] && (
                  <span className="pill blue">{this.tiers[a.tierId].name}</span>
                )} */}
              </td>
              <td>
                {a.userGroupIds.map(
                  id =>
                    this.groups[id] && (
                      <span className="pill white" key={id}>
                        {this.groups[id].name}
                      </span>
                    ),
                )}
      ...
      // case 'tiers':
      //   return this.props.tiers.map(t => (
      //     <tr key={t.id}>
      //       <td>
      //         <input
      //           type="checkbox"
      //           checked={tierIds.includes(t.id)}
      //           onChange={this.onTierChange(t.id)}
      //         />
      //       </td>
      //       <td className="attendee-selector__name">{t.name}</td>
      //       <td className="attendee-selector__name">
      //         {t.attendeeCount} attendee{t.attendeeCount > 1 ? 's' : ''}
      //       </td>
      //     </tr>
      //   ))
      case 'groups':
          ...
          <tr key={g.id}>
            <td>
              <input
                ...
                onChange={this.onGroupChange(g.id)}
              />
            </td>
  ...
  menuHeaderText = () => {
    ...
    if (tierIds.length) {
      const count = tierIds.reduce(
        (count, tierId) => count + (this.tiers[tierId] ? this.tiers[tierId].attendeeCount : 0),
        0,
      )
  ...
  classNameForMenuItem = view =>
    `attendee-selector__main-menu-item${view === this.state.view ? '--selected' : ''}`
  ...
  onAttendeeChange = id => event =>
    event.target.checked ? this.addAttendeeId(id) : this.removeAttendeeId(id)

  onTierChange = id => event => (event.target.checked ? this.addTierId(id) : this.removeTierId(id))

  onGroupChange = id => event =>
    event.target.checked ? this.addGroupId(id) : this.removeGroupId(id)
...
export function isAttendeeInTierOrGroup(attendee, tierIds, groupIds) {
  return (
    tierIds.includes(attendee.tierId) || !!groupIds.find(g => attendee.userGroupIds.includes(g))
  )
}
